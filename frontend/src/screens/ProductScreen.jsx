...
const ProductScreen = ({match}) => {
  ...
  return (
    <>
      ...
      <Meta title={product.title} description='' keywords=''/>
      ...
      <Layout title='Product details' description={product.title}>
        <Container>
          ...
          <Paper elevation={10} square={false}
                 style={{paddingInline: '8px', minHeight: '600px', marginTop: '1rem', marginBottom: '2rem'}}>
            <Grid container spacing={2}>
              <Grid item xs={12} md={6}>
                <Image src={`${window.location.origin}/${product.picturePath}`} style={{minHeight: '457px'}} fluid
                       rounded/>
              </Grid>
              ...
              <Grid item xs={12} md={3}>
                <Card>
                  <ListGroup variant='flush'>
                    ...
                    <ListGroupItem>
                      <Button className='btn-block' onClick={addToCartHandler} type='button' variant={'contained'}
                              color={'primary'}>
                        Add To Cart
                      </Button>
                    </ListGroupItem>
